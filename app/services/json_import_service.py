...
from app.models import Course, CourseTee, CourseHole, Round, RoundHole, Shot, Club, Player
...
def import_courses(db: Session, courses: list[ParsedCourseRef]) -> dict:
    """Upsert courses by garmin_snapshot_id."""
    created, updated, skipped = 0, 0, 0
...
def _create_round(sc: ParsedScorecard, db: Session) -> Round:
    ...
    if not course and sc.course_name:
        course = db.query(Course).filter(Course.name == sc.course_name).first()
...
def import_full_export(db: Session, parsed: dict) -> dict:
    """
